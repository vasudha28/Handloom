...
const signUpSchema = z.object({
  ...
  // GST number is required for B2B customers
  if (data.role === 'b2b_buyer') {
  ...
  // Company name is required for B2B customers
  if (data.role === 'b2b_buyer') {
...
export default function FloatingAuthModal({ isOpen, onClose, defaultTab = 'signin' }: FloatingAuthModalProps) {
  ...
  // Update form role when customer type changes
  useEffect(() => {
    const newRole = customerType === 'individual' ? 'customer' : 'b2b_buyer';
    signUpForm.setValue('role', newRole);
  }, [customerType, signUpForm]);
  ...
  const handleSignUp = async (data: any) => {
    ...
    try {
      await register({
        ...
        role: customerType === 'individual' ? 'customer' : 'b2b_buyer',
            ...
            <Tabs value={currentTab} onValueChange={(value) => setCurrentTab(value as 'signin' | 'signup')}>
              ...
              <TabsContent value="signup" className="space-y-4">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-customer-type">Account Type</Label>
                    <Select 
                      onValueChange={(value: 'individual' | 'b2b') => {
                  ...
                  {/* Individual Customer - Google Signup */}
                  {customerType === 'individual' && (
                    <div className="text-center space-y-4">
                      <div className="space-y-2">
                        <h4 className="font-medium">Individual Customer</h4>
                        <p className="text-sm text-muted-foreground">
                  ...
                  {/* B2B Customer - Email/Password Signup */}
                  {customerType === 'b2b' && (
                    <form onSubmit={signUpForm.handleSubmit(handleSignUp)} className="space-y-4">
                      <div className="space-y-2">
                        <h4 className="font-medium">B2B Customer Registration</h4>
                        <p className="text-sm text-muted-foreground">
                          Create your business account with email and password
                        </p>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {signUpForm.formState.errors.companyName && (
                          <p className="text-sm text-red-500">{signUpForm.formState.errors.companyName.message}</p>
                        )}
                      </div>
                      ...
                      <Button type="submit" className="w-full" disabled={loading}>
                        {loading ? "Creating account..." : "Create B2B Account"}
                      </Button>
                    </form>
                  )}
